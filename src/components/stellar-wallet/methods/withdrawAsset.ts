...
import { Transaction, Keypair, Account, TransactionBuilder, BASE_FEE, Networks, Operation, Asset, Memo, MemoHash } from 'stellar-sdk';
...
export default async function withdrawAsset(e: Event) {
  try {
    ...
    if (hasCurrency === -1) await this.trustAsset(null, currency[0], currency[1], pincode);

    const info = await axios.get(`${this.toml.TRANSFER_SERVER}/info`).then(({ data }) => data);
    ...
    const interactive = await axios
      .post(`${this.toml.TRANSFER_SERVER}/transactions/withdraw/interactive`, formData, {
        headers: {
    ...
    await new Promise((resolve, reject) => {
      ...
      window.onmessage = ({ data: { transaction } }) => {
        ...
        if (transaction.status === 'completed') {
          ...
          this.loading = { ...this.loading, withdraw: false };
          resolve();
        } else if (!submittedTxn && transaction.status === 'pending_user_transfer_start') {
          this.server
            .accounts()
