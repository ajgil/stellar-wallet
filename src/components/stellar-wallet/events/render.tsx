  ...
  return [
    ...
    this.account ? (
      [
        ...
        <button class={this.loading.deposit ? 'loading' : null} type="button" onClick={e => this.depositAsset(e)}>
          {this.loading.deposit ? <stellar-loader /> : null} Deposit Asset
        </button>,
        <button class={this.loading.withdraw ? 'loading' : null} type="button" onClick={e => this.withdrawAsset(e)}>
          {this.loading.withdraw ? <stellar-loader /> : null} Withdraw Asset
        </button>,

        <button class={this.loading.trust ? 'loading' : null} type="button" onClick={e => this.trustAsset(e)}>
          {this.loading.trust ? <stellar-loader /> : null} Trust Asset
        </button>,
        <button class={this.loading.pay ? 'loading' : null} type="button" onClick={e => this.makePayment(e)}>
          {this.loading.pay ? <stellar-loader /> : null} Make Payment
        </button>,
      ]
    ) : (
      <button class={this.loading.fund ? 'loading' : null} type="button" onClick={e => this.createAccount(e)}>
    ...
    this.account
      ? [
          <button class={this.loading.update ? 'loading' : null} type="button" onClick={e => this.updateAccount(e)}>
